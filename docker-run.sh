...
case "$1" in
  ...
  -c|--compile|compile)
    docker run --rm -v "$(pwd)":/site/app $IMAGE 'bundle exec nanoc'
    ;;

  -s|--server|server)
    docker run --rm -v "$(pwd)":/site/app -p 4000:4000 $IMAGE 'bundle exec nanoc view -p 4000'
    ;;

  -g|--guard|guard)
    docker run --rm -v "$(pwd)":/site/app -i -t $IMAGE 'guard'
    ;;
